...
#Training our XGBoost Model on the Training Set
...
#Applying Grid Search to optimise parameters
from sklearn.model_selection import GridSearchCV
paramaters = {'n_estimators': [50, 100, 200, 300, 500], 
              'max_depth': [2, 6, 10, 14, 18, 22, 26, 30, None], 
              'subsample': [0.1, 0.25, 0.5, 0.75, 1],
              'colsample_bylevel': [0.1, 0.25, 0.5, 0.75, 1],
              'colsample_bytree': [0.1, 0.25, 0.5, 0.75, 1],
              'reg_lamba': [1, 5, 25, 50, 75, 100], 
              'min_child_weight': [1, 5, 25, 50, 75, 100], 
              'learning_rate': [0.1, 0.25, 0.5, 0.75, 1]}
grid_search = GridSearchCV(estimator = regressor, 
                           param_grid = paramaters, 
                           scoring = 'accuracy',
                           cv = 10,
                           n_jobs = -1) #n_jobs number of process you want your computer to use, -1 means all processes
